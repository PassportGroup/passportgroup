...
<template>
  <div>
    <div v-if="$page.props.flash.success && show" class="mb-3 flex items-center justify-between bg-green-500 rounded max-w-3xl">
      <div class="flex items-center">
        ...
        <div class="py-4 text-white text-sm font-medium">{{ $page.props.flash.success }}</div><p class="ml-2" v-if="extra_msg">{{ extra_msg }}</p>
      </div>
    ...
    <div v-if="($page.props.flash.error || ($page.props.errors && Object.keys($page.props.errors).length > 0)) && show" class="my-2 flex items-center justify-between bg-red-400 rounded max-w-3xl">
      <div class="flex items-center">
        ...
        <div v-if="$page.props.errors && Object.keys($page.props.errors).length >= 1" class="py-4 text-white text-sm font-medium">
          {{ $tc('general.number-of-form-errors', Object.keys($page.props.errors).length) }}
        </div>
      </div>
      <button type="button" class="group mr-2 p-2" @click="show = false">
